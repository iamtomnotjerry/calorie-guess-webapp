  ...
  for (const modelName of MODELS) {
    try {
      ...
      const prompt = language === 'Tiếng Việt' 
        ? `Phân tích hình ảnh món ăn này. 
           YÊU CẦU PHẢN HỒI DƯỚI DẠNG JSON (VÀ CHỈ JSON) theo cấu trúc sau:
           {
             "dishName": "Tên món ăn cụ thể",
             "calories": { "value": số_calo, "unit": "kcal" },
             "macros": [
               { "name": "Protein", "value": "số_gam+g", "details": "Nguồn gốc (VD: từ thịt sườn, trứng)" },
               { "name": "Carbs", "value": "số_gam+g", "details": "Nguồn gốc (VD: từ gạo tấm)" },
               { "name": "Fat", "value": "số_gam+g", "details": "Nguồn gốc (VD: từ mỡ hành, dầu chiên)" }
             ],
             "healthAssessment": {
               "positives": ["Ưu điểm 1", "Ưu điểm 2"],
               "negatives": ["Hạn chế 1", "Hạn chế 2"],
               "advice": "Lời khuyên ngắn gọn để ăn lành mạnh hơn"
             }
           }
           Lưu ý: 
           - Ước tính calo phải chính xác theo khẩu phần thực tế trong hình, KHÔNG ước tính quá cao (non-overrated).
           - TẤT CẢ nội dung văn bản phải bằng Tiếng Việt.`
        : `Analyze this food image. 
           REQUIRE RESPONSE IN JSON FORMAT (AND ONLY JSON) with this structure:
           {
             "dishName": "Specific dish name",
             "calories": { "value": calo_number, "unit": "kcal" },
             "macros": [
               { "name": "Protein", "value": "grams+g", "details": "Source info" },
               { "name": "Carbs", "value": "grams+g", "details": "Source info" },
               { "name": "Fat", "value": "grams+g", "details": "Source info" }
             ],
             "healthAssessment": {
               "positives": ["Point 1", "Point 2"],
               "negatives": ["Issue 1", "Issue 2"],
               "advice": "Short healthy eating advice"
             }
           }
           Note: 
           - Be accurate based on portion size, do NOT overestimate (non-overrated).
