...
export async function analyzeImage(base64Image: string, language: string = 'Tiếng Việt', foodDescription?: string) {
  // Get identifying info for logging
  const headersList = await headers();
  const ip = headersList.get('x-forwarded-for') || 'Unknown IP';
  const userAgent = headersList.get('user-agent') || 'Unknown Device';
  
  console.log(`>>> New analysis request from IP: ${ip} | Description: ${foodDescription || 'None'}`);
  ...
  // 🚀 Model rotation updated based on your available_models.json:
  const MODELS = [
...
${foodDescription ? `Người dùng cung cấp thông tin thêm về món ăn: "${foodDescription}". Hãy kết hợp thông tin này với hình ảnh để phân tích.` : 'Hãy phân tích **chính xác** hình ảnh món ăn này dựa trên khẩu phần thực tế nhìn thấy (không được ước lượng quá cao, không phóng đại).'}
...
${foodDescription ? `User description of the food: "${foodDescription}". Combine this info with the image for analysis.` : 'Analyze this food image **accurately** based on the visible real portion size. Do NOT overestimate calories or macros.'}
